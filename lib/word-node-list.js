...
class WordNodeList {
  ...
  _removeFromSortedList (node, distance) {
    ...
    while (beginAtIndex === null) {
      ...
      [fits, direction] = beginsSeriesAtDistance(sl, bIndex, distance);
      if (fits) {
        beginAtIndex = bIndex;
      } else if (direction === -1) {
  ...
  _addToSortedList (node, distance) {
    ...
    if (!sl.length || sl[sl.length - 1].distance <= distance) {
      ...
      this._lastInsertion = { distance, index: sl.length - 1 };
      ...
      this._lastInsertion = { distance, index: 0 };
    } else {
      ...
      // chase. If we got `startIndex` from reading
      // `this._indexAndDistanceOfLastInsertion`, there's a decent chance that
      // our first attempt will be a valid insertion position.
      let fits, direction;
      [fits] = fitsAtIndex(sl, startIndex, distance);
      if (fits) {
      ...
      while (insertAtIndex === null) {
        ...
        [fits, direction] = fitsAtIndex(sl, bIndex, distance);
        if (fits) {
      ...
      this._lastInsertion = { distance, index: insertAtIndex };
    }
  }
