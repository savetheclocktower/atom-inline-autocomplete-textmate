...
const InlineAutocompleteTextmate = {
  ...
  activate () {
    ...
    let observer = atom.workspace.observeTextEditors(editor => {
      ...
      let onChange = () => {
        if (!this.ignoring) { this.reset(); }
      };

      disposables.add(
        editor.onDidChangeCursorPosition(onChange)
      );
      disposables.add(
        editor.onDidSave(onChange)
      );
    });
    ...
    disposables.add(
      atom.commands.add('atom-text-editor', {
        'inline-autocomplete-textmate:stop': () => { this.reset(); },
        'inline-autocomplete-textmate:cycle': (e) => {
          if (this.shouldIgnore()) {
            return e.abortKeyBinding();
          }
          this.toggleAutocomplete(e, 1);
        },
        'inline-autocomplete-textmate:cycle-back': (e) => {
  ...
  isValidAutocompletePosition () {
    // Ensure we have a word character adjacent to the cursor on at least one
    // side.
  ...
  // side of the cursor or selection.
  //
  ...
  // lorem i[psu]m dolor.         -> prefix: 'i',     suffix: 'm'
  //
  getPrefixAndSuffixOfSelection (selection) {
    ...
    this.buffer.scanInRange(RE_WORD, lRange, ({ match, range, stop }) => {
      let [fullMatch] = match;
